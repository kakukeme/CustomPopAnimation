...
{
    // interactivePopTransition就是我们说的方法2返回的对象，我们需要更新它的进度来控制Pop动画的流程，
    // 我们用手指在视图中的位置与视图宽度比例作为它的进度。
    
    CGFloat progress = [recognizer translationInView:recognizer.view].x / recognizer.view.bounds.size.width;
    
    // 稳定进度区间，让它在0.0（未完成）～1.0（已完成）之间
    progress = MIN(0, MAX(0.0, progress));
...
// 苹果提供给我们的用来重写，控制器之间的转场动画 (pop 或者 push)(nav里的pop和push)
...
// 苹果让我们返回一个交互的对象，用来实时管理控制器之间转场动画的完成度，
// 通过它我们可以让控制器的转场动画与用户交互（注意一点，如果方法1返回是nil，方法2是不会调用的，也就是说，只有我们自定义的动画才可以与控制器交互）。
